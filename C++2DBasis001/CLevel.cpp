...
bool CLevel::LoadMapFile(const char* file_path)
{
    ...
    for (int nCntMapH = 0; nCntMapH < HeightMax; nCntMapH++)
    {
        ...
        for (int nCntMapW = 0; nCntMapW < WidthMax; nCntMapW++)
        {
            ...
            //ブロックの設置	
            case 0:
				CBlock::Create(D3DXVECTOR3(m_BlockWidth + (m_BlockWidth * nCntMapW), m_BlockHeight + (m_BlockHeight * nCntMapH) ,0.f), D3DXVECTOR2(m_BlockWidth * 2, m_BlockHeight * 2), CBlock::BlockType::NORMAL);
                break;
